...
from losses.parse_and_id.id_loss import id_loss
from losses.parse_and_id.parsing_loss import parse_loss
from losses.fft_loss import fft_loss
from datasets.data_utils import get_dataset, get_dataloader
from configs.paths_config import DATASET_PATHS, MODEL_PATHS
from datasets.imagenet_dic import IMAGENET_DIC

from models.HFGI.load_model import HyperHFGI, Adapter
from models.HFGI.load_model import EqualConv2d, ScaledLeakyReLU

from models.gan_direction.e4e_d.e4e_d import E4EDirection
...
class Asyrp(object):
    def __init__(self, args, config, device=None):
        ...
        if self.args.gan_edit:
            self.gan = E4EDirection(self.args).to(self.device)
    ...
    def load_pretrained_model(self):
        ...
        if self.config.data.dataset in ["CelebA_HQ", "LSUN", "CelebA_HQ_Dialog"]:
            ...
            model = DDPM(self.config, self.args)
            ...
            # model是Unet噪声预测模型, init_ckpt是预训练Diffusion模型

            # model = i_DDPM(self.config.data.dataset)
            model = i_DDPM(self.config.data.dataset, self.args)
            if self.args.model_path:
                init_ckpt = torch.load(self.args.model_path)
            else:
    ...
    def run_training(self):
        ...
        if self.args.multi_id_loss_w > 0:
            self.multi_id_loss_func = id_loss.IDLoss().to(self.device).eval()
        if self.args.parse_loss_w > 0:
            self.parse_loss_func = parse_loss.ParseLoss().to(self.device).eval()
        if self.args.fft_loss_w > 0:
            self.fft_loss_func = fft_loss.fftLoss().eval()
        ...
        if self.args.gan_edit:
            ...
            self.target_features = clip_loss_func.get_text_features(self.trg_txts[0], templates=templates)
            self.target_features = self.target_features.mean(axis=0, keepdim=True)
            self.target_features = self.target_features / self.target_features.norm(dim=-1, keepdim=True)  #  (1, 512)

        # ----------- Get seq -----------#
        # !说明: DDIM采样采用的是均匀采样
        # n_train_step: 用于构建DDIM训练步（S_for->S_edit）,训练过程中的采样步数，需要根据t_edit进行截断（因为只有>t_edit才需要训练）
        # n_test_step: 用于构建DDIM测试（生成）用多少步（论文中的S_for）,测试过程中的采样步数，不需要根据t_edit进行截断
        # n_inv_step: 反演（生成）用多少步（论文中的S_for）
        # seq_train/test/inv :存储的是时间步t
        ...
        if self.args.train_delta_block:
            # setattr_layers就是论文中的ft，get_h_num表示不同属性，为每个属性训练一个ft网络
            ...
            for i in range(self.args.get_h_num):
                # 获得delta_h, 不同地layer_{i}表示不同地属性
                get_h = getattr(model.module, f"layer_{i}")
                optim_param_list = optim_param_list + list(get_h.parameters())
            if self.args.gan_edit:
                optim_param_list += list(self.gan.parameters())
                optim_param_list += list(model.module.parameters())
        ...
        for it_out in range(self.args.start_iter_when_you_use_pretrained ,self.args.n_iter):
            ...
            exp_dir = os.path.split(self.args.exp)[-2]
            exp_dir = os.path.split(exp_dir)[-1]
            if self.args.load_from_checkpoint:
                ...
                if not os.path.exists(f'checkpoint/{exp_dir}'):
                    os.makedirs(f'checkpoint/{exp_dir}')
                save_name = f'checkpoint/{exp_dir}/{exp_id}_{it_out}.pth'
            ...
            if self.args.do_train:
                ...
                if self.args.retrain==0 and os.path.exists(save_name):
                    ...
                    # img_lat_pairs(train/test):原图、加噪又去噪的原图，全噪声图
                    for step, (x0, x_rec, x_lat) in enumerate(img_lat_pairs_dic['train']):
                        if self.args.target_image_id:
                        ...
                        if self.args.train_delta_block:
                            for i in range(self.args.get_h_num):
                                ...
                                for p in get_h.clip_T.parameters():
                                    p.requires_grad = False
                                for p in get_h.clip_T.model.parameters():
                                    p.requires_grad = False
                                if self.args.gan_edit:
                                    for p in self.gan.parameters():
                                        p.requires_grad = True
                                    for p in self.gan.net.parameters():
                                        p.requires_grad = False
                                    model.module.alpha.requires_grad = False
                        ...
                        x0_tensor = x0_tensor.to(self.device).detach()
                        ...
                        with tqdm(total=len(seq_train), desc=f"training iteration") as progress_bar:
                            for t_it, (i, j) in enumerate(zip(reversed(seq_train), reversed(seq_train_next))):
                                ...
                                # denoising_step函数
                                # 其中t_next应该是t-1，因此才表示预测下一步的去噪过程
                                # 但在precompute_pairs函数中，t_next=t_prev，即t_next是t+1，表示的是一个加噪的过程
                                # 因此可以看出加噪并不是随机采样噪声、而是同样使用Unet网络预测噪声进行计算t+1
                                # 这其实是the ODE approximation of its reversal as the forward process，它与DDIM采样公式一摸一样，只是把t-1变成了t+1
                                # 因此代码中加噪过程只需要denoising_step函数把t_next=t_next换成t_next=t_prev即可

                                gan_direction = None
                                if self.args.gan_edit:
                                    gan_direction = self.gan(x0_tensor, self.target_features.float())

                                    
                                # 论文中Algorithm2 row.15
                                # step 1: Asyrp
                                # (最后一步直接x_next复制为了x0_t)?
                                xt_next, x0_t, _, _ = denoising_step(xt_next.detach(), t=t, t_next=t_next, models=model,
                                                            logvars=self.logvar,                                        
                                                            b=self.betas,
                                                            sampling_type=self.args.sample_type,
                                                            eta=0.0,
                                                            learn_sigma=self.learn_sigma,  # index表示有index+1个属性需要同时编辑，index=none表示不进行编辑
                                ...
                                if self.args.multi_id_loss_w > 0:
                                    loss_multi_id = self.multi_id_loss_func(x0_t, x0_t_origin)
                                    loss += self.args.multi_id_loss_w * loss_multi_id
                                if self.args.parse_loss_w > 0:
                                    loss_parse = self.parse_loss_func(x0_t, x0_t_origin)
                                    loss += self.args.parse_loss_w * loss_parse
                                if self.args.fft_loss_w > 0:
                                    loss_fft = self.fft_loss_func(x0_t, x0_t_origin)
                                    loss += self.args.fft_loss_w * loss_fft


                                loss.backward()
                                optim_ft.step()

                                progress_bar.set_description(f"{step}-{it_out}: loss_clr: {loss_clr:.3f} loss_l1: {loss_l1:.3f} loss_id: {loss_id:.3f} loss_clip:{loss_clip:.5f} loss_multi_id:{loss_multi_id:.3f} loss_parse:{loss_parse:.3f} loss_fft:{loss_fft:.3f} loss: {loss:.3f} ")
                    ...
                    if self.args.train_delta_block:
                        for i in range(self.args.get_h_num):
                            ...
                            dicts[f"{i}"] = get_h.state_dict()
                        if self.args.gan_edit:
                            dicts["gan"] = self.gan.state_dict()
                            dicts["alpha"] = model.module.alpha
                    if self.args.train_delta_h:
                        for key in delta_h_dict.keys():
                            dicts[f"{key}"] = delta_h_dict[key]
            ...
            if self.args.do_test:
                ...
                for step, (x0, x_rec, x_lat) in enumerate(img_lat_pairs_dic['test']):
                    if x_lat_tensor is None:
                        x_lat_tensor = x_lat
                        if self.args.use_x0_tensor:
                            x0_tensor = x0
                    ...
                    self.save_image(model, x_lat_tensor, seq_test, seq_test_next,
                                                ...
                                                file_name=f'test_{step}_{it_out}', hs_coeff=hs_coeff,
                                                x_rec=x_rec,
                                                )
    ...
    def save_image(self, model, x_lat_tensor, seq_inv, seq_inv_next,
                    ...
                    image_space_noise_dict=None, x_rec=None):
        ...
        process_num = int(save_x_origin) + (len(hs_coeff) if isinstance(hs_coeff, list) else 1)   # 2
        ...
        with tqdm(total=len(seq_inv)*(process_num), desc=f"Generative process") as progress_bar:
            ...
            # No delta h
                ...
                for it, (i, j) in enumerate(zip(reversed((seq_inv)), reversed((seq_inv_next)))):
                    t = (torch.ones(self.args.bs_train) * i).to(self.device)
                    t_next = (torch.ones(self.args.bs_train) * j).to(self.device)  # t-1
                ...
                if self.args.save_inv:
                    x_inv = x
                    if self.args.bs_train != 1:
                        for i in range(self.args.bs_train):
                            x_inv_s = x_inv[i:i+1,:]
                            tvu.save_image((x_inv_s + 1) * 0.5, os.path.join(self.args.inv_image_folder, f'{file_name}_ngen{self.args.n_train_step}_{i}.png'), normalization=True)
                    else:
                        tvu.save_image((x_inv + 1) * 0.5, os.path.join(self.args.inv_image_folder, f'{file_name}_ngen{self.args.n_train_step}.png'), normalization=True)
                ...
                for hs_coeff_tuple in hs_coeff:
                    ...
                    for it, (i, j) in enumerate(zip(reversed((seq_inv)), reversed((seq_inv_next)))):
                        ...
                        x, x0_t, delta_h, _ = denoising_step(x, t=t, t_next=t_next, models=model,
                                        ...
                                        b=self.betas,
                                        learn_sigma=self.learn_sigma,
                                        # 这里index一直是0没有问题，因为model进行forward时，如果t < t_edit，是不会进行编辑的，即et == et_modified
                                        ...
                                        eta=1.0 if t[0]<self.t_addnoise else 0.0,    # 此处控制生成过程不同阶段
                                        t_edit= self.t_edit,
        ...
        if self.args.save_src_and_gen:
            x_gen_single = x
            if self.args.bs_train != 1:
                for i in range(self.args.bs_train):
                    x0_s = x0_tensor[i:i+1,:]
                    x_gen_s = x_gen_single[i:i+1,:]
                    tvu.save_image((x0_s + 1) * 0.5, os.path.join(self.args.src_image_folder, f'{file_name}_ngen{self.args.n_train_step}_{i}.png'), normalization=True)
                    tvu.save_image((x_gen_s + 1) * 0.5, os.path.join(self.args.gen_image_folder, f'{file_name}_ngen{self.args.n_train_step}_{i}.png'), normalization=True)

            else:
                tvu.save_image((x0_tensor + 1) * 0.5, os.path.join(self.args.src_image_folder, f'{file_name}_ngen{self.args.n_train_step}.png'), normalization=True)
                tvu.save_image((x_gen_single + 1) * 0.5, os.path.join(self.args.gen_image_folder, f'{file_name}_ngen{self.args.n_train_step}.png'), normalization=True)
    ...
    def run_test(self):
        ...
        # 用于记录推理过程中保存的图片，是在多少train_step下训练的模型生成的
        if self.args.n_train_step != 0:
            seq_train = np.linspace(0, 1, self.args.n_train_step) * self.args.t_0
            seq_train = seq_train[seq_train >= self.t_edit]
        ...
        seq_test_edit = [int(s+1e-6) for s in list(seq_test_edit)]  # test_edit，这里好像没啥用
        seq_test = [int(s+1e-6) for s in list(seq_test)]
        seq_test_next = [-1] + list(seq_test[:-1])  # test
            ...
            if self.args.train_delta_block:
                ...
                # delta_block load
                    ...
                    for i in range(self.args.get_h_num):
                        ...
                        if self.args.gan_edit:
                            self.gan.load_state_dict(torch.load(save_name_list[i])["gan"])
                            model.module.alpha = (torch.load(save_name_list[i])["alpha"])
    ...
    # ----------- Pre-compute -----------#
    # 每次执行此代码得到tran/test两个模型的image_pair——x0：原图, x：加噪又去噪后的反演图/重建图, x_lat：全噪声图/最后一步加到的噪声图
    @torch.no_grad()
    def precompute_pairs(self, model, save_imgs=False):
        ...
        for mode in ['train', 'test']:
            ...
            if os.path.exists(pairs_path) and not self.args.re_precompute:
                ...
                # 数据集获取
                # 对于FFHQ，最多只能选择500张图，而且DATASET_PATHS下必须是图片的直接目录
                # 只有这里用到了获取数据集操作
                ...
                with torch.no_grad():
                    ...
                    with tqdm(total=len(seq_inv), desc=f"Inversion process {mode} {step}") as progress_bar:
                        for it, (i, j) in enumerate(zip((seq_inv_next[1:]), (seq_inv[1:]))):
                            ...
                            # 只生成噪声图
                            # x_tnext, x0_t
                            # x_999, 即x被加噪到最后一步，一张全噪声图（这是一个加噪过程）
                    ...
                    x_lat = x.clone()
                    # ?可以通过这一步保存图像来检查一下x_lat到底是个什么：一个全噪声图
                    if save_imgs:
                    ...
                    with tqdm(total=len(seq_inv), desc=f"Generative process {mode} {step}") as progress_bar:
                        for it, (i, j) in enumerate(zip(reversed((seq_inv)), reversed((seq_inv_next)))):
                            ...
                            # 生成噪声图的同时还生成了当前t步的x0预测，最终生成重建图x0
                            # x被去噪到最后一步，
                    ...
                    # x0：原图, x：加噪又去噪后的反演图/重建图, x_lat：全噪声图/最后一步加到的噪声图
    ...
    # script_get_lpips只是执行这个函数计算LPIPS
    # 通过LPIPS计算t_edit和t_addnoise是set_t_edit_t_addnoise()函数
    @torch.no_grad()
    def compute_lpips_distance(self):
        ...
        for step, img in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                time_e = time.time()
                print(f'{time_e - time_s} seconds')    # precompute计算LPIPS
    ...
    def set_t_edit_t_addnoise(self, LPIPS_th=0.33, LPIPS_addnoise_th=0.1, return_clip_loss=False):
        ...
        # t_edit is from LPIPS(x0_t, x0)    获取t_edit
        ...
        # t_boost is from LPIPS(xt, x0)    获取t_addnoise
