...
from models.learning_prompt import LP
from models.learning_matrix import LM
...
class HyperBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels, temb_channels=512, args=None, device=None):
        ...
        self.conv = torch.nn.Conv2d(out_channels,
                                     out_channels,
                                     kernel_size=1,
                                     stride=1,
                                     padding=0)
    ...
    def forward(self, x, temb):
        ...
        h = h + self.temb_proj(torch.sigmoid(temb))[:, :, None, None]
...
class DDPM(nn.Module):
    ...
    def setattr_layers(self, nums):
        ...
        for i in range(nums):
            ...
            #                            dropout=0.0)
            # )
            setattr(self, f"layer_{i}", HyperBlock(in_channels=block_in,
                                        out_channels=block_in,
                                        temb_channels=self.temb_ch,
                                        args=self.args,
                                        )
    ...
    def forward(self, x, t, index=None, t_edit=400, conditions=None, hs_coeff=(1.0, 1.0), w=None, delta_h=None, gan_direction=None, plist=None, ignore_timestep=False, use_mask=False):
                ...
                if delta_h is None: #Asyrp
                    ...
                    for i in range(index+1):
                        ...
                        if not self.args.gan_edit:
                            ...
                            delta_h_gan = gan_direction.view(1, 512, 1, 1)
                            # mysig = torch.sigmoid(self.alpha)
