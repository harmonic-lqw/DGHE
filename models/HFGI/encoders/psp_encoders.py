...
from models.HFGI.encoders.helpers import get_blocks, bottleneck_IR, bottleneck_IR_SE, _upsample_add
from models.HFGI.stylegan2.model import EqualLinear,ScaledLeakyReLU,EqualConv2d
...
class ResidualEncoder(Module):
    ...
    def forward(self, x):
        ...
        feat3 = self.conv_layer3(feat2)  # (N, 64, 64, 64)


        scale = self.condition_scale3(feat3)  # (N, 512, 64, 64)
        scale = torch.nn.functional.interpolate(scale, size=(64,64) , mode='bilinear')  # (N, 512, 64, 64)
        conditions.append(scale.clone())
        shift = self.condition_shift3(feat3)  # (N, 512, 64, 64)
        shift = torch.nn.functional.interpolate(shift, size=(64,64) , mode='bilinear')  # (N, 512, 64, 64)
