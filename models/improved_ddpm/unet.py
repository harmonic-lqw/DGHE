...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        # torch.sigmoid 后，系数就变成了 0.5
        if self.args.gan_edit:
            self.alpha = th.nn.Parameter(th.tensor(0.0))
    ...
    def forward(self, x, timesteps, y=None, index=None, t_edit=400, hs_coeff=(1.0, 1.0), delta_h=None, ignore_timestep=False, use_mask=False, gan_direction=None, plist=None, conditions=None, w=None):
        """
        ...
        if index is not None:
            ...
            if timesteps[0] >= t_edit:
                ...
                if delta_h is None: #Asyrp
                    ...
                    for i in range(index+1):
                        ...
                        if not self.args.gan_edit:
                            h2 += delta_h * hs_coeff[i+1]
                        else:
                            # delta_h(1, 512, 8, 8)   gan_direction(1, 512)
                            delta_h_gan = gan_direction.view(self.args.bs_train, 512, 1, 1)
                            # mysig = th.sigmoid(self.alpha)
                            mysig = self.args.mysig
                            edit_h = mysig * delta_h + (1 - mysig) * delta_h_gan
                            h2 += edit_h * hs_coeff[i+1]
            ...
            hs_index = -1
            # out_blocks之前: h2 (1, 512, 8, 8)
            for module in self.output_blocks:
                h2 = th.cat([h2, hs[hs_index]], dim=1)
                hs_index -= 1
                h2 = module(h2, emb)
            h2 = h2.type(x.dtype)  # (1, 128, 256, 256)

            h2 = self.out(h2)  # (1, 6, 256, 256)
        ...
        # h这里已经经过上采样变成了噪声
        # 因此这里返回的：不带条件（delta_h）预测的噪声、带条件预测的噪声、ft预测的delta_h、Unet最中间的特征图h
        # h:  torch.Size([1, 6, 256, 256])
        # h2:  torch.Size([1, 6, 256, 256])
        # delta_h:  torch.Size([1, 512, 8, 8])
        # middle_h:  torch.Size([1, 512, 8, 8])
    ...
    #     # print("="*30)
    #     # print("self.channel_mult: ", self.channel_mult)  #  (1, 1, 2, 2, 4, 4)
    #     # print("self.model_channels: ", self.model_channels)  #  128
    #     # print("self.num_res_blocks: ", self.num_res_blocks)  #  1

    #     # 一路下采样到最中间的特征图，self.channel_mult、self.model_channels、self.num_res_blocks
    ...
    def setattr_layers(self, nums):
        ...
        for i in range(nums):
            setattr(self, f"layer_{i}", HyperBlock(in_channels=ch,
                                        out_channels=ch,
                                        temb_channels=self.model_channels * 4,
                                        args=self.args
                                        )
...
class HyperBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels, temb_channels=512, args=None, device=None):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.temb_proj = th.nn.Linear(temb_channels,
                                        out_channels)
        self.args = args
        self.cross_attention = CrossAttention(d_model=512, nhead=4, dim_feedforward=1024)
        if device is None:
            device = th.device("cuda") if th.cuda.is_available() else th.device("cpu")
        self.device = device

        self.conv = th.nn.Conv2d(out_channels,
                                     out_channels,
                                     kernel_size=1,
                                     stride=1,
                                     padding=0)
        
        self.target_txts = SRC_TRG_TXT_DIC[self.args.edit_attr][1][0]

        self.clip_T = CLIPLoss(
            self.device,
            lambda_direction=0,
            lambda_patch=0,
            lambda_global=0,
    ...
    def forward(self, x, temb):
        ...
        h = h + self.temb_proj(th.sigmoid(temb))[:, :, None, None]

        # fix prompt
        if self.args.learning_prompt:
            target_features = self.clip_T()
        # learning prompt
        elif self.args.learning_matrix:
            target_features = self.clip_T()
        else:
            templates = ['a photo of a {}.']
            target_features = self.clip_T.get_text_features(self.target_txts, templates=templates)  # (len(templates), 512)

        target_features = target_features.mean(axis=0, keepdim=True)
        target_features = target_features / target_features.norm(dim=-1, keepdim=True)  #  (1, 512)
        
        target_features = target_features.unsqueeze(1).permute(1, 0, 2)  #  (1, 1, 512):K/V
        target_features = target_features.float()

        B, C, H, W = h.shape
        h = h.flatten(2).permute(2, 0, 1)  # (64, 1, 512):Q
        target_features = target_features.repeat(1, self.args.bs_train, 1)
        h = self.cross_attention(h, target_features, no_res=True)
        h = h.transpose(-2, -1).contiguous().view(B, C, H, W)  # (1, 512, 8, 8)
