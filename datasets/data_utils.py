...
from .CelebA_HQ_dataset import get_celeba_dataset, get_celeba_dataset2
from .LSUN_dataset import get_lsun_dataset, get_lsun_dataset2
...
class CustomImageDataset(Dataset):
    def __init__(self, img_dir, transform=None, test_nums=None, train=True):
        ...
        if test_nums is not None:
            if train:
                self.img_files = self.img_files[:-test_nums]  # 从0到最后第test_nums张
            else:
                self.img_files = self.img_files[-test_nums:]  # 最后test_nums张
...
def get_dataset(dataset_type, dataset_paths, config, target_class_num=None, gender=None):
    ...
    if dataset_type == 'AFHQ':
        ...
        if config.data.category == "church_outdoor":
            train_dataset, test_dataset = get_lsun_dataset2(dataset_paths['LSUN_church'], config)
        ...
        train_dataset, test_dataset = get_celeba_dataset2(dataset_paths['CelebA_HQ'], config)
        ...
        train_dataset = CustomImageDataset(os.path.join(dataset_paths['MetFACE'],'images'), transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]), test_nums=1000)
        test_dataset = CustomImageDataset(os.path.join(dataset_paths['MetFACE'],'images'), transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]), test_nums=1000, train=False)
    elif dataset_type == "FFHQ":
        train_dataset = CustomImageDataset(dataset_paths['FFHQ'], transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]), test_nums=1000)
        test_dataset = CustomImageDataset(dataset_paths['FFHQ'], transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]), test_nums=1000, train=False)
    else:
        raise ValueError
