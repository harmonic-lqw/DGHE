...
class LSUN_dataset(data.Dataset):
    def __init__(self, image_root, transform=None, mode='train', img_size=256):
        ...
        if mode == 'train':
            self.image_paths = glob(os.path.join(image_root, 'church_outdoor_train', '*.jpg'))
        elif mode == 'test':
            self.image_paths = sorted(glob(os.path.join(image_root, 'church_outdoor_val2', '*.jpg')))
...
def get_lsun_dataset2(data_root, config):
    ...
    train_dataset = LSUN_dataset(data_root, transform=train_transform, mode='train',
                                 img_size=config.data.image_size)
    test_dataset = LSUN_dataset(data_root, transform=test_transform, mode='test',
                                img_size=config.data.image_size)
