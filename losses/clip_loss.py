...
class CLIPLoss(torch.nn.Module):
    ...
    def get_text_features(self, class_str: str, templates=imagenet_templates, norm: bool = True) -> torch.Tensor:
        ...
        tokens = clip.tokenize(template_text).to(self.device) # (79, 77)

        text_features = self.encode_text(tokens).detach()  # (79, 512)

        if norm:
            text_features /= text_features.norm(dim=-1, keepdim=True)  # (79, 512)
            
        return text_features  # (79, 512)  !

    def get_image_features(self, img: torch.Tensor, norm: bool = True) -> torch.Tensor:
        # print(img.size()) # (1, 3, 256, 256)
        image_features = self.encode_images(img)  # (1, 512)
    ...
    def compute_text_direction(self, source_class: str, target_class: str) -> torch.Tensor:
        ...
        text_direction = (target_features - source_features).mean(axis=0, keepdim=True)  # (1, 512)
        text_direction /= text_direction.norm(dim=-1, keepdim=True)  # (1, 512)
    ...
    def clip_directional_loss(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str) -> torch.Tensor:
        if self.target_direction is None:
            self.target_direction = self.compute_text_direction(source_class, target_class)  #  (1, 512)

        src_encoding    = self.get_image_features(src_img)
        target_encoding = self.get_image_features(target_img)

        edit_direction = (target_encoding - src_encoding)
        edit_direction /= (edit_direction.clone().norm(dim=-1, keepdim=True) + 1e-7)  #  (1, 512)
