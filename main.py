...
def parse_args_and_config():
    ...
    parser.add_argument('--multi_id_loss_w', type=float, default=0, help='Weights of Multi ID loss')
    parser.add_argument('--parse_loss_w', type=float, default=0, help='Weights of Parse loss')
    parser.add_argument('--fft_loss_w', type=float, default=0, help='Weights of FFT loss')
    ...
    parser.add_argument('--save_src_and_gen', action="store_true", default=False, help='Whether to save src and gen single')
    parser.add_argument('--save_inv', action="store_true", default=False, help='Whether to save inv single')
    parser.add_argument('--adapter_learning_rate', default=0.0001, type=float, help='Adapter optimizer learning rate')
    parser.add_argument('--mod_adapter', action="store_true", help="Whether use adapter to modulation")
    parser.add_argument('--t_modulation', type=int, default=1000, help="Which step use feature modulation")

    parser.add_argument('--learning_prompt', action="store_true", help="Whether use learning prompt")
    parser.add_argument('--learning_matrix', action="store_true", help="Whether use learning matrix")
    parser.add_argument('--gan_edit', action="store_true", help="Whether use gan inversion to find edit direction")

    parser.add_argument('--mysig', type=float, default=0.5, help="Manual adjustment of mysig")


    # e4e model
    parser.add_argument('--e4e_model', action="store_true", help="Whether use gan inversion to edit U-net with p123")
    parser.add_argument('--gan_model_path', type=str, default=None, help='the path of gan inversion model')
    ...
    # 实验结果保存路径（推理的时候保存的文件夹名字根据设置的超参数创建，而非根据ckpt名字创建)
    ...
    args.gen_image_folder = os.path.join(args.exp, 'gen_images')
    if not os.path.exists(args.gen_image_folder):
        os.makedirs(args.gen_image_folder)
    args.src_image_folder = os.path.join(args.exp, 'src_images')
    if not os.path.exists(args.src_image_folder):
        os.makedirs(args.src_image_folder)

    args.inv_image_folder = os.path.join(args.exp, 'inv_images')
    if not os.path.exists(args.inv_image_folder):
        os.makedirs(args.inv_image_folder)
